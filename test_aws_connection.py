...
def test_aws_credentials():
    ...
    print("=" * 60)
    print("[CREDENTIALS] Testing AWS Credentials...")
    print("=" * 60)
...
def test_s3_access():
    ...
    try:
        ...
        if error_code == 'AccessDenied':
            print("[WARN] S3 Access: LIMITED (no ListBuckets permission)")
...
def test_sagemaker_access():
    ...
    print("\n" + "=" * 60)
    print("[SAGEMAKER] Testing SageMaker Access...")
    print("=" * 60)
    ...
    try:
        ...
        print(f"[PASS] SageMaker Access: OK")
...
def test_iam_role():
    ...
    print("\n" + "=" * 60)
    print("[IAM] Testing IAM Role for SageMaker...")
    print("=" * 60)
    ...
    try:
        ...
        try:
            ...
            role = response['Role']
            print(f"[PASS] IAM Role '{role_name}': FOUND")
            print(f"   Role ARN: {role['Arn']}")
            print(f"   Use this ARN in your .env file as SAGEMAKER_ROLE")
            return role['Arn']
        except ClientError as e:
            if e.response['Error']['Code'] == 'NoSuchEntity':
                print(f"[WARN] IAM Role '{role_name}': NOT FOUND")
                print("   You need to create a SageMaker execution role")
                print("   Follow the docs/AWS_SETUP_GUIDE.md Step 2")
            else:
                print(f"[FAIL] IAM Role check failed: {e}")
            return None
    except ClientError as e:
        if e.response['Error']['Code'] == 'AccessDenied':
            print("[WARN] IAM Access: Cannot check roles (missing IAM permissions)")
...
def test_sagemaker_endpoint_invoke(endpoint_name='mobile-price-predictor-endpoint'):
    ...
    print("\n" + "=" * 60)
    print(f"[ENDPOINT] Testing SageMaker Endpoint Invocation...")
    print("=" * 60)
    ...
    try:
        ...
        print(f"[PASS] Endpoint '{endpoint_name}': WORKING")
        ...
        if error_code == 'ValidationError' and 'Could not find endpoint' in str(e):
            print(f"[WARN] Endpoint '{endpoint_name}': NOT FOUND")
            print("   This is expected if you haven't deployed a model yet")
            print("   Deploy a model first using deploy_to_sagemaker.py")
        else:
            print(f"[FAIL] Endpoint invocation failed: {e}")
        return False
    except Exception as e:
        print(f"[FAIL] Endpoint invocation failed: {e}")
        return False
...
def main():
    print("\n" + "=" * 60)
    print(" AWS Integration Test - Mobile Price Predictor")
    print("=" * 60)
    ...
    if not results['credentials']:
        ...
        print("   Follow docs/AWS_SETUP_GUIDE.md Step 2")
    elif not results['endpoint']:
        print("1. Deploy a model to create an endpoint:")
        print("   Use deploy_to_sagemaker.py to train and deploy")
